...
#     #subtract the other random shit you have to: f(3)m(2), f(2)m(3), f(n)m(n+1), f(n+1)m(n)
#     #signs are flipped from whiteboard because you are subtracting twice, T = c- sum, sum = newsum - this
#     rv += (f(n+1,3,m)*mm(2,m) + f(n+1,n+1,m)*mm(n+2,m)) % m
#     rv -= (f(n+1,2,m)*mm(3,m) + f(n+1,n+2,m)*mm(n+1,m)) % m

#     x0,x1,x2 = 0,0,0
#     #sqrt trick application for odd sum
#     for k in range(3, isqrt(n+1)+1):
#         #print(k)
#         x0 += F0((n+1)//k, m)*(pow(2, k-1, m) - 1)
#         x1 += F1((n+1)//k, m)*(k*(pow(2, k-1, m) - 1))
#         x2 += F2((n+1)//k, m)*(k*k*(pow(2, k-1, m) - 1))

#         x0 %= m
#         x1 %= m
#         x2 %= m
#     # fixed for n//z - n//(z+1) values
#     for z in range(isqrt(n+1), 0, -1):
#         k = n//z
#         if k == isqrt(n+1): continue
#         x0 += F0((n+1)//k, m)*(sum2tok((n+1)//z, m) - sum2tok((n+1)//(z+1), m))
#         x1 += F1((n+1)//k, m)*(sumk2tok((n+1)//z, m) - sumk2tok((n+1)//(z+1), m))
#         x2 += F2((n+1)//k, m)*(sumkk2tok((n+1)//z, m) - sumkk2tok((n+1)//(z+1), m))
