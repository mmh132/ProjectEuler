...
def w(a, b, c):
    ...
    for tr in range(1, 2*min(a, b) + 1):
        ...
        # then for c's we have tr + 2 transition holes
        # as well as al a-holes, and bl b-holes
        rv += comb(A + al - 1, A - 1)*comb(B + bl - 1, B - 1)*comb(cl + tr + al + bl + 1, al + bl + tr + 1)
    return rv % (10**9 + 7)
